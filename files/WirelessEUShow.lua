...
--从存档文件中读取上次关机前的数据
local fileInit = io.open("save.txt","a")
if fileInit then
fileInit:close()
end

local fileLoad = io.open("save.txt","r")
if fileLoad then
    for line in fileLoad:lines() do
    table.insert(EU_Monitor.secondEU,1,line)
    end
    fileLoad:close()
end
...
function EU_Monitor.update()
    ...
    if file then
        for _, value in ipairs(EU_Monitor.secondEU) do
            file:write(value, "\n")
        end
        file:close()
    end
